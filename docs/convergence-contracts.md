...
| `AGENT-1`       | Auto Drive, external agents, and Background Review        | Intentionally owned                          | Every Code orchestration, explicit external-agent preflight, durable review state, duplicate suppression, and budget cancellation remain product-owned.                                                                                               | `codex-rs/core/tests/suite/external_agent_preflight.rs` (`wrong_copilot_executable_fails_explicit_preflight`, `missing_external_agent_command_fails_explicit_preflight_with_install_hint`, `logged_out_claude_agent_fails_explicit_preflight_as_authentication_required`, `external_command_agent_routes_through_spawn_agent_with_provider_provenance`); `codex-rs/core/src/agent/external_preflight_tests.rs`; `codex-rs/core/tests/suite/review.rs`; `codex-rs/core/tests/suite/auto_review.rs`                                                                                                                           |
