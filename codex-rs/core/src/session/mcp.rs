...
impl Session {
    ...
    pub(crate) async fn runtime_mcp_config_and_context(
        ...
        let execution_context = self.services.execution_account.snapshot().await;
    ...
    pub(crate) async fn refresh_mcp_if_dirty(self: &Arc<Self>) {
        ...
        loop {
            let auth = self.services.execution_account.auth_manager().auth_cached();
            if !self
                .services
                .mcp_runtime
            ...
            let mcp_projection = self
                ...
                .runtime_config_for_step(
                    ...
                    &self.services.thread_extension_data,
                    &desired.originator,
                    PluginAuthContext::from_auth(desired.execution_context.auth.as_ref()),
                    &ready_selected_capability_roots,
                    executor_capability_discovery.as_deref(),
                )
                .await;
    ...
    pub(crate) async fn refresh_mcp_servers_now(
        ...
        let mcp_projection = self
            ...
            .await;
        let mut refresh_config = refresh_config.clone();
        refresh_config.permissions.approval_policy = turn_context.approval_policy.clone();
        desired.config = Arc::new(refresh_config);
        self.publish_mcp_runtime(
            &desired,
            mcp_projection,
            &ready_selected_capability_roots,
