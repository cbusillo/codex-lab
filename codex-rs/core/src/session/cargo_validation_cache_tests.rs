...
#[tokio::test]
async fn maintenance_bounds_cleanup_lock_wait() {
    let home = tempdir().expect("create cache home");
    let repository = tempdir().expect("create repository");
    let lease = acquire(
        &home,
    ...
    timeout(Duration::from_secs(1), result_rx)
        .await
        .expect("maintenance should not wait for the cleanup lock")
        .expect("maintenance result should be reported")
        .expect("contended maintenance should be a graceful fallback");
    assert!(cleanup_pending_path(root).exists());
...
async fn maintenance_retries_cleanup_after_temporary_contention() {
    ...
    timeout(Duration::from_secs(1), result_rx)
        .await
        .expect("maintenance should finish after lock release")
        .expect("maintenance result should be reported")
        .expect("temporary contention should preserve cache enforcement");
...
#[tokio::test]
async fn timed_out_maintenance_hands_cleanup_to_active_holder() {
    let home = tempdir().expect("create cache home");
    let repository = tempdir().expect("create repository");
    let limits = CargoValidationCacheLimits {
        max_entries: 4,
    ...
    let waiting_key = (0..512_u32)
        ...
        .find(|key| {
            ...
            lock_path(&root, key) != lock_path(&root, &active_key)
        })
    ...
    .await;
    let artifact = waiting.target_dir().join("oversized");
    fs::write(&artifact, "12345").expect("write oversized artifact");
    ...
    cleanup_lock.lock_exclusive().expect("hold cleanup lock");
    let (result_tx, result_rx) = tokio::sync::oneshot::channel();
    spawn_maintenance(waiting, move |result| {
        let _ = result_tx.send(result);
    });

    let marker_deadline = tokio::time::Instant::now() + Duration::from_millis(100);
    while !cleanup_pending_path(&root).exists() && tokio::time::Instant::now() < marker_deadline {
        tokio::time::sleep(Duration::from_millis(5)).await;
    }
    clear_cleanup_pending(&root).expect("clear marker before the first cleanup pass");
    prune_cache_locked(&root, &key, limits).expect("run first cleanup pass");
    assert!(artifact.exists());

    timeout(Duration::from_secs(1), result_rx)
        .await
        .expect("waiting maintenance should remain bounded")
        .expect("maintenance result should be reported")
        .expect("waiting maintenance should hand cleanup to the active holder");
    assert!(cleanup_pending_path(&root).exists());

    let mut entry_lock = Some(entry_lock);
    assert!(matches!(
        cleanup::finish_cache_cleanup_pass(&root, &mut entry_lock, &cleanup_lock, Ok(()))
            .expect("active cleanup should accept the handoff"),
        cleanup::CacheCleanupPass::Repeat
    ));
    prune_cache_locked(&root, &key, limits).expect("run handed-off cleanup pass");
    assert!(!artifact.exists());
    assert!(matches!(
        cleanup::finish_cache_cleanup_pass(&root, &mut entry_lock, &cleanup_lock, Ok(()))
            .expect("active cleanup should finish"),
        cleanup::CacheCleanupPass::Complete
    ));
    assert!(!cleanup_pending_path(&root).exists());
}
...
async fn blocking_cache_preparation_is_explicitly_bounded() {
    ...
    assert!(started.elapsed() < Duration::from_secs(1));
    let second = bounded_blocking(&cancellation, "second test preparation", move || {
        second_started_tx.send(()).map_err(io::Error::other)?;
        Ok(())
    });
    assert!(timeout(Duration::from_millis(75), second).await.is_err());
