...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    ...
    let result = Session::new(
        ...
        environment_manager,
        Arc::new(project_validation_coordinator::ProjectValidationCoordinator::default()),
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let services = SessionServices {
        ...
        project_validation_coordinator: Arc::new(
            project_validation_coordinator::ProjectValidationCoordinator::default(),
        ),
        project_validation_success_cache:
            project_validation_coordinator::ProjectValidationSuccessCache::default(),
        ...
        control_models_manager: Arc::clone(&models_manager),
...
#[tokio::test]
async fn build_initial_context_omits_multi_agent_v2_usage_hints_when_agents_are_disabled() {
    let (session, turn_context) =
...
async fn wait_for_flush_count(
    ...
    match result {
        Ok(calls) => calls,
        Err(_) => panic!(
            "store should observe {expected_flushes} flushes; calls: {:?}",
            store.calls().await
        ),
    }
}
...
async fn remote_compaction_v2_retains_only_the_selected_step(first_attempt: FirstAttempt) {
    ...
    crate::compact_remote_v2::run_inline_remote_auto_compact_task(
        ...
        &mut client_session,
        crate::compact::CompactionJobConfig {
            initial_context_injection: InitialContextInjection::DoNotInject,
            model_request_history_mode: crate::context_manager::ModelRequestHistoryMode::Normal,
            trigger: codex_analytics::CompactionTrigger::Auto,
...
async fn turn_aborted_flushes_terminal_event_after_delivery() {
    ...
    // The task runner may also reach its own flush before abort finalization takes ownership.
    let calls = wait_for_flush_count(&store, /*expected_flushes*/ 2).await;
    assert!(
        (2..=3).contains(&calls.flush_thread),
        "unexpected abort flush count: {calls:?}"
    );
}
...
async fn active_turn_keeps_first_root_when_mail_coalesces(inherited_root: Option<&str>) {
    ...
    .enumerate()
    {
        sess.input_queue
            ...
            .await;
        if index == 0 {
            // The queued first message supplies the new task root; the second
            // arrives after that root is established and cannot replace it.
            sess.spawn_task(
                Arc::clone(&tc),
                Vec::new(),
                NeverEndingTask {
    ...
    assert_eq!(
        ...
        Some(inherited_root.unwrap_or("root-a"))
    );
...
async fn abort_review_task_emits_exited_then_aborted_and_records_history() {
    ...
    let history = sess.clone_history().await;
    let expected_role = match tc.multi_agent_version {
        MultiAgentVersion::Disabled | MultiAgentVersion::V1 => "user",
        MultiAgentVersion::V2 => "developer",
    };
