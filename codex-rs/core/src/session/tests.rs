...
fn step_settings_for_test(
    config: &crate::config::Config,
    collaboration_mode: CollaborationMode,
    service_tier: Option<String>,
) -> Arc<super::step_settings::StepSettings> {
    Arc::new(super::step_settings::StepSettings {
        collaboration_mode,
...
pub(crate) fn update_turn_settings_for_test(
    ...
    update: impl FnOnce(&mut ResolvedStepSettings),
) {
...
impl StepContext {
    pub(crate) fn for_test(turn: Arc<TurnContext>) -> Arc<Self> {
        ...
        Arc::new(Self {
            settings,
            token_budget: turn.config.token_budget.clone(),
            model_info: Arc::new(turn.model_info.clone()),
...
#[test]
fn resolve_multi_agent_version_preserves_history_and_legacy_fallbacks() {
...
async fn set_rate_limits_retains_previous_credits() {
    ...
    let session_configuration = SessionConfiguration {
        ...
        service_tier: None,
        step_settings: step_settings_for_test(&config, collaboration_mode, None),
...
#[tokio::test]
async fn build_initial_context_omits_multi_agent_v2_usage_hints_when_version_disabled() {
    let (session, mut turn_context) =
