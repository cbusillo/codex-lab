...
{
    tokio::task::spawn_blocking(move || report(maintain_cache(lease)));
}
    ...
    F: FnOnce() -> io::Result<T> + Send + 'static,
{
    ...
    tokio::select! {
        ...
        result = tokio::time::timeout(CACHE_PREPARE_TIMEOUT, task) => match result {
            Ok(result) => result
                .map_err(|error| io::Error::other(format!("cargo cache {operation} task failed: {error}")))?
                .map(Some),
            Err(_) => Ok(None),
        }
    }
...
fn maintain_cache(lease: CargoValidationCacheLease) -> io::Result<()> {
    ...
    // Waiting here serializes overlapping finish tasks without delaying or
    // replacing the validation result, and ensures a deferred entry is measured
    // once the other task releases its entry lock.
...
fn enforce_cache_bounds(
    ...
    let result = prune_cache_locked(root, key, limits);
    // Keep cleanup exclusive until the current entry is no longer active. A
    // queued maintenance pass can then measure this entry instead of deferring
    // it as blocked and leaving the bounds unenforced until unrelated reuse.
    drop(entry_lock);
    combine_with_unlock(result, &cleanup_lock, "cargo validation cache cleanup lock")
}
