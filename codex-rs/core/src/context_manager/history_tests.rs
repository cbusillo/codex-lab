...
fn for_prompt_strips_images_when_model_does_not_support_images() {
    let items = vec![
        ...
        ResponseItem::FunctionCall {
            id: Some("function-call-1".to_string()),
        ...
        ResponseItem::FunctionCallOutput {
            id: Some("function-output-1".to_string()),
            call_id: "call-1".to_string(),
        ...
        ResponseItem::CustomToolCall {
            id: Some("custom-call-1".to_string()),
            status: None,
        ...
        ResponseItem::CustomToolCallOutput {
            id: Some("custom-output-1".to_string()),
            call_id: "tool-1".to_string(),
            name: None,
            output: FunctionCallOutputPayload::from_content_items(vec![
                FunctionCallOutputContentItem::InputText {
...
fn record_items_truncates_function_call_output_content() {
    ...
    match &history.items[0] {
        ResponseItem::FunctionCallOutput { id, output, .. } => {
            assert_eq!(id.as_deref(), Some("output-1"));
...
fn record_items_truncates_custom_tool_call_output_content() {
    ...
    assert_eq!(history.items.len(), 1);
    match &history.items[0] {
        ResponseItem::CustomToolCallOutput { id, output, .. } => {
            assert_eq!(id.as_deref(), Some("output-2"));
