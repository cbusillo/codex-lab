...
impl Session {
    ...
    pub(crate) async fn maybe_start_turn_for_pending_work_with_sub_id(
        ...
        if !self.input_queue.has_pending_mailbox_items().await
            || (!has_trigger_turn && !self.has_outstanding_durable_sleep())
        {
            return;
        }
        ...
        let inherited_cyber_access_program = if has_trigger_turn {
            None
        } else {
        ...
        let mut turn_context = self.new_default_turn_with_sub_id(sub_id).await;
        if turn_context.config.model_provider_id == codex_model_provider_info::OPENAI_PROVIDER_ID {
            Arc::get_mut(&mut turn_context)
                .expect("new turn context should be uniquely owned before publication")
                .cyber_access_program = inherited_cyber_access_program;
        }
