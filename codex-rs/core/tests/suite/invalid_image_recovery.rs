...
async fn invalid_image_recovery_clears_all_candidate_images_in_one_pass() -> Result<()> {
    ...
    test.codex
        ...
        .await?;
    let recovery_event = wait_for_event(&test.codex, |event| {
        matches!(event, EventMsg::Error(_) | EventMsg::TurnComplete(_))
    })
    .await;
    assert!(
        matches!(recovery_event, EventMsg::Error(_)),
        "expected invalid-image error before turn completion, got {recovery_event:?}; requests={}",
        mock.requests().len()
    );
    wait_for_event(&test.codex, |event| {
        matches!(event, EventMsg::TurnComplete(_))
