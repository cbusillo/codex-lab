...
/// This test writes to a fallback credentials file in CODEX_LAB_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire CODEX_LAB_HOME through the code.
#[test]
#[serial(codex_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 3: seed an isolated CODEX_LAB_HOME with fallback OAuth tokens for this
