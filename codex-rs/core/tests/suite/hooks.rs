...
fn code_mode_custom_tool_output_text_and_success(output_item: &Value) -> (String, Option<bool>) {
    match output_item.get("output") {
        ...
        Some(Value::Object(output)) => (
            output
                ...
                .unwrap_or_default()
                .to_string(),
            output.get("success").and_then(Value::as_bool),
        ),
...
"#
    ...
    let (output, success) = code_mode_custom_tool_output_text_and_success(&output_item);
    assert_ne!(
        success,
        Some(false),
        "rewritten nested tool result should leave the code cell successful: {output}"
    );
    assert!(
...
"#
    ...
    assert_ne!(
        success,
        Some(false),
        "caught pre-tool block should leave the code cell successful: {output}"
    );
...
"#
    ...
    assert_ne!(
        success,
        Some(false),
        "caught post-tool block should leave the code cell successful: {output}"
    );
    assert!(output.contains(r#""kind":"caught""#));
    assert!(output.contains(reason));
    assert!(!output.contains("unexpected-success"));
...
#[tokio::test]
async fn post_tool_use_continue_false_keeps_code_mode_tool_promise_resolved() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let server = start_mock_server().await;
    let call_id = "posttooluse-code-mode-continue-false";
    let marker_dir = TempDir::new().context("create post tool feedback marker directory")?;
    let marker = marker_dir.path().join("continue-false");
    let command = format!(
        "printf executed > {}; printf original-post-tool-result",
        marker.display()
    );
...
try {{
  const result = await tools.exec_command({{ cmd: {command_json} }});
  text(JSON.stringify({{ kind: "resolved", result }}));
}} catch (error) {{
  text(JSON.stringify({{ kind: "unexpected-error", error: String(error) }}));
}}
"#
    );
    let responses = mount_sse_sequence(
    ...
    let feedback = "feedback only after nested result";
    let mut builder = test_codex()
        .with_model("test-gpt-5.1-codex")
        .with_pre_build_hook(move |home| {
            if let Err(error) =
                write_post_tool_use_hook(home, Some("^Bash$"), "continue_false", feedback)
            {
                panic!("failed to write feedback-only post tool use hook fixture: {error}");
            }
        })
        .with_config(|config| {
            let _ = config.features.enable(Feature::CodeMode);
    ...
    test.submit_turn_with_permission_profile(
        "run the shell command with feedback after execution from code mode",
        PermissionProfile::Disabled,
    )
    .await?;
    ...
    assert_ne!(
        success,
        Some(false),
        "feedback-only post hook should leave the code cell successful: {output}"
    );
    assert!(output.contains(r#""kind":"resolved""#));
    assert!(output.contains("original-post-tool-result"));
    assert!(!output.contains("unexpected-error"));
    assert_eq!(
        fs::read_to_string(&marker).context("read feedback-only post tool marker")?,
        "executed"
    );
