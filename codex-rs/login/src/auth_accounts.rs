        ...
        match account.mode {
            ...
            AuthMode::Headers => {
                return Err(io::Error::other(
                    "stored header account activation is not supported",
                ));
            }
            AuthMode::AgentIdentity => {
                return Err(io::Error::other(
            ...
            AuthMode::BedrockApiKey => {
                return Err(io::Error::other(
                    "stored Bedrock API key account activation is not supported",
                ));
            }
        };
...
fn restore_previous_activation(
    ...
    let auth_result = restore_previous_auth(
        codex_home,
        previous_auth,
        auth_credentials_store_mode,
        keyring_backend_kind,
