...
fn test_host_skills_service(
    codex_home: &TempDir,
    bundled_skills_enabled: bool,
) -> HostSkillsService {
    HostSkillsService::new_with_home_dir(
        codex_home.path().abs(),
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let skills_service =
        test_host_skills_service(&codex_home, /*bundled_skills_enabled*/ true);
...
async fn snapshot_for_config_merges_extension_host_and_legacy_plugin_roots() {
    ...
    let skills_service =
        test_host_skills_service(&codex_home, /*bundled_skills_enabled*/ false);
