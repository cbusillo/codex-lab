...
impl McpConnectionSet {
    ...
    pub async fn new(
        ...
        let (codex_apps_auth_provider, codex_apps_auth_discriminator) = codex_apps_auth
            .map(|context| {
                (
                    Some(context.provider),
                    Some(context.connection_discriminator),
                )
            })
            .unwrap_or((None, None));
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            let connection_identity = McpServerConnectionIdentity::new(
                ...
                (server_name == CODEX_APPS_MCP_SERVER_NAME && runtime_auth_provider.is_some())
                    .then(|| codex_apps_auth_discriminator.clone())
                    .flatten(),
