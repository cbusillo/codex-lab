...
pub(super) async fn updates_check(config: &Config) -> DoctorCheck {
    updates_check_for_build(config, codex_version::is_lab_build()).await
}

async fn updates_check_for_build(config: &Config, is_lab_build: bool) -> DoctorCheck {
    if is_lab_build {
...
mod tests {
    ...
    async fn codex_lab_disabled_update_ui_snapshot() {
        ...
        let report = DoctorReport {
            ...
            checks: vec![updates_check_for_build(&config, /*is_lab_build*/ true).await],
        };
    ...
    async fn codex_lab_skips_desktop_update_enrichment() {
        ...
        let mut checks = vec![updates_check_for_build(&config, /*is_lab_build*/ true).await];
