...
pub struct InProcessClientStartArgs {
    ...
    /// Structured launch provenance recorded in app-server thread metadata.
...
impl InProcessAppServerClient {
    ...
    /// Callers may await request completion without concurrently draining
    /// notifications; all events remain queued in order for later consumption.
    ...
    ///
    /// Events are buffered independently from request processing, so callers may
    /// await requests before consuming any resulting notifications.
...
mod tests {
    ...
    async fn unread_lossless_notifications_do_not_block_in_process_requests() {
        let mut client = timeout(
            ...
            start_test_client_with_capacity(SessionSource::Cli, /*channel_capacity*/ 1),
        )
        .await
        .expect("in-process client startup should finish");
        let thread: ThreadStartResponse = timeout(
        ...
        .await
        .expect("thread/start should finish")
        .expect("thread/start should succeed");
