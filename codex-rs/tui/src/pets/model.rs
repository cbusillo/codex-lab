...
//! in-memory `Pet`: built-in catalog ids, custom pet ids under CODEX_LAB_HOME,
...
impl Pet {
    ...
    /// has already been materialized into CODEX_LAB_HOME; if callers skip the
...
fn load_builtin_pet(pet: catalog::BuiltinPet, codex_home: Option<&Path>) -> Result<Pet> {
    let codex_home = codex_home.context("CODEX_LAB_HOME is not available")?;
