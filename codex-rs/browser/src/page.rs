...
async fn wait_ready_state(
    ...
    loop {
        let state = cdp_page.evaluate(script).await.ok().and_then(|r| {
            r.value()
...
"#;
                    ...
                    if fallback_navigated {
                        ...
                        .await
                        {
                            ...
                            Ok(Err(e)) => {
                                ...
                                .await;
                                if !url_looks_loaded(&self.cdp_page, Duration::from_millis(400))
                                    .await
                                {
  ...
  try {{
    ...
    pub async fn execute_javascript(&self, code: &str) -> Result<serde_json::Value> {
        ...
        let user_code_literal = serde_json::to_string(&user_code_with_source).map_err(|err| {
            BrowserError::CdpError(format!("failed to serialize user code: {err}"))
        })?;
        let wrapped = format!(
            r#"(async () => {{
  ...
  try {{
    ...
    const __userCode = {user_code_literal};
