...
impl BrowserLaunchLockFile {
    async fn acquire(timeout: Duration) -> Result<Self> {
        let lock_path = std::env::temp_dir().join("codex-browser-launch.lock");
...
fn resolve_chrome_log_path() -> Option<PathBuf> {
    ...
    let base = if let Ok(home) = std::env::var("CODE_HOME").or_else(|_| std::env::var("CODEX_HOME"))
    {
        ...
        return Some(std::env::temp_dir().join("codex-chrome.log"));
    };

    let path = base.join("codex-chrome.log");
...
impl BrowserManager {
    ...
    async fn capture_screenshot_regular(&self) -> Result<(Vec<std::path::PathBuf>, String)> {
        ...
        let assets = assets_guard
            .as_ref()
            .ok_or_else(|| {
                BrowserError::AssetError("asset manager was not initialized".to_string())
            })?
            .clone();
        drop(assets_guard);
