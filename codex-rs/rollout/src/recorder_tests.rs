...
#[tokio::test]
async fn reopening_writer_resyncs_paginated_ordinal_from_disk() -> std::io::Result<()> {
    let home = TempDir::new().expect("temp dir");
    let rollout_path = home.path().join("rollout.jsonl");
    ...
    let mut state = RolloutWriterState::new(
        ...
        home.path().to_path_buf(),
        rollout_path,
        RolloutOrdinalState::Paginated { next: Some(2) },
    );

    state.ensure_writer_open().await?;

    assert_eq!(state.ordinal_state.current()?, Some(5));
    Ok(())
}

#[tokio::test]
async fn reopening_empty_writer_keeps_paginated_ordinal_state() -> std::io::Result<()> {
    let home = TempDir::new().expect("temp dir");
    let rollout_path = home.path().join("rollout.jsonl");
    File::create(&rollout_path)?;
    let mut state = RolloutWriterState::new(
        ...
        home.path().to_path_buf(),
        rollout_path,
        RolloutOrdinalState::Paginated { next: Some(0) },
    );

    state.ensure_writer_open().await?;

    assert_eq!(state.ordinal_state.current()?, Some(0));
    Ok(())
}

#[tokio::test]
async fn paginated_recorder_assigns_zero_based_ordinals() -> std::io::Result<()> {
    let home = TempDir::new().expect("temp dir");
    let config = test_config(home.path());
    let recorder = RolloutRecorder::new(
        &config,
    ...
    .await?;
    recorder
        .record_canonical_items(&[agent_message_item("first"), agent_message_item("second")])
        .await?;
    recorder.flush().await?;

    let lines = read_rollout_lines(recorder.rollout_path())?;
    assert_eq!(
        lines.iter().map(|line| line.ordinal).collect::<Vec<_>>(),
        vec![Some(0), Some(1), Some(2)]
    );
    let RolloutItem::SessionMeta(session_meta) = &lines[0].item else {
